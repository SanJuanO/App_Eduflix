...
import UIKit
import RNCryptor
class DetalleForosViewController:
  ...
  var idarraycursos:[Int] = []
    ...
    func consultacursos(){
        ...
        let datos_a_enviar = ["idcurso": busqueda as Int] as NSMutableDictionary

        //ejecutamos la función arrayFromJson con los parámetros correspondientes (url archivo .php / datos a enviar)
        let dataJsonUrlClass = ConexionPost2()
        dataJsonUrlClass.arrayFromJson(url2:"getByIdCurso",datos_enviados:datos_a_enviar){ (datos_recibidos) in
...
if let dictionary = datos_recibidos as? [String: Any] {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.titulo.text = self.bnombre[indexPath.item]
        cell.entrega.text = "Fecha " + self.bfechar[indexPath.item]
...
let           password = "Secretpasswordiim"
        ...
        let contenido = "\(id)"+",dforos,"+"\(self.busqueda)"+",\(self.bid[indexPath.item])"
