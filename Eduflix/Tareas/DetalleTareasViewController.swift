...
import UIKit
import RNCryptor
class DetalleTareasViewController:
  ...
  var idarraycursos:[Int] = []
    ...
    func consultacursos(){
       let id_ = UserDefaults.standard.string(forKey: "id")!
        ...
        let datos_a_enviar = ["id_usuario": id! as Int,"idcurso": busqueda as Int] as NSMutableDictionary
        ...
        dataJsonUrlClass.arrayFromJson(url2:"Tareas/getByIdUsuarioAndIdCurso",datos_enviados:datos_a_enviar){ (datos_recibidos) in
...
if let dictionary = datos_recibidos as? [String: Any] {
                    ...
                    if let array = dictionary["datos"] as? NSArray {
                        ...
                        for obj in array {
                            ...
                            if let dict = obj as? NSDictionary {
                                ...
                                self.horario.append(dict.value(forKey: "horario") as! String)
                                let f2 = dict.value(forKey: "fechar") as! String + "+0000"
                             
                                let fecha2 = dateFormatter2.date(from:f2)
                                                
                                self.fechar.append(dateFormatter.string(from:fecha2! ) )
                                if (dict.value(forKey: "fecha") as? String) != nil
                                {
                                    
                                    let f = dict.value(forKey: "fecha") as! String + "+0000"
                                 
                                    let fecha1 = dateFormatter2.date(from:f)
                                                                     
                                    self.fecha.append(dateFormatter.string(from:fecha1! )  )
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.fecha.text = "Día " +  self.fechar[indexPath.item]
        
        cell.titulo.text = self.bnombre[indexPath.item]
        cell.entrega.text = "Entrega " + self.bfecha[indexPath.item] + "a las " + self.bhorario[indexPath.item]
...
let           password = "Secretpasswordiim"
        ...
        let contenido = "\(id)"+",tarea,"+"\(self.busqueda)"+",\(self.bid[indexPath.item])"
